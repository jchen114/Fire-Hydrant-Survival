...
public class WaterPumper : MonoBehaviour, IManageable {
	...
	private float levelDecreaseAmount = 0.5f;
	private float minimumDecreaseAmount = 2.0f;
	...
	void MobileTouchHandler() {
		...
		foreach (Touch touch in Input.touches) {
			...
			if (touchStartedOnHydrant && touch.phase == TouchPhase.Moved) {
				HandleTouchWhilePressed(pos);
			}

			if (touchStartedOnHydrant && touch.phase == TouchPhase.Ended) {
				HandleTouchEnd (pos);
			}
...
//			print (string.Format("Finger id = {0}, x = {1}, y = {2}", touch.fingerId, touch.position.x, touch.position.y));
	...
	public void IncreaseWaterLevel() {
		decreaseAmount -= levelDecreaseAmount;
		if (decreaseAmount <= minimumDecreaseAmount) {
			decreaseAmount = minimumDecreaseAmount;
		}
	}
